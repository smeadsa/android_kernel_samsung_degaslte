...
#if 0 /* temporarily blocked - fled not implemented */
...
static bool rt5033_chg_init(struct rt5033_charger_data *charger)
{
    ...
    /* Set switching frequency to 0.75MHz */
    rt5033_set_bits(charger->rt5033->i2c_client,
                    RT5033_CHG_CTRL1, RT5033_SEL_SWFREQ_MASK);
	/* Enable High-GM */
	rt5033_set_bits(charger->rt5033->i2c_client,
			0x07, 0x80);
	...
	/* Enable realtime battery detect, battery detect, battery type detect */
	data = rt5033_reg_read(charger->rt5033->i2c_client, RT5033_CONFIG_H);
	data |= 0xE;
	rt5033_reg_write(charger->rt5033->i2c_client, RT5033_CONFIG_H, data);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		/* TEMP_TEST : CURRENT_MAX & CURRENT_NOW is the same one? */
        val->intval = 2000;
		break;
	...
	case POWER_SUPPLY_PROP_PRESENT:
		/*data = rt5033_reg_read(charger->rt5033->i2c_client, RT5033_CONFIG_L);
		data &= ~(0x02);
		pr_info("[RT5033] Battery (%d)\n", data);
		val->intval = data;*/
		break;
	default:
		return -EINVAL;
...
{
	struct rt5033_charger_data *charger = data;
	...
	rt5033_enable_charger_switch(charger, 0);
	safe_ldo = regulator_get(charger->rt5033->dev, "rt5033_safe_ldo");
...
static irqreturn_t rt5033_chg_ieoc_irq_handler(int irq, void *data)
{
	...
	pr_info("%s : EOC CNT = %d / %d\n", __func__,
         charger->eoc_cnt, EOC_DEBOUNCE_CNT);
    if (charger->eoc_cnt >= EOC_DEBOUNCE_CNT) {
...
{
	rt5033_charger_data_t *charger = container_of(to_delayed_work(work),
					   rt5033_charger_data_t, current_sinking_work);
    struct i2c_client *iic = charger->rt5033->i2c_client;
    int ctrl4,ctrl5;
